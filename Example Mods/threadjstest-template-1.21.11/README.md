...
### 6) More Side‑by‑Side Examples

Below are additional “regular Java Fabric” vs “ThreadJS” examples for common tasks.
...
ServerTickEvents.END_SERVER_TICK.register(server -> {
		// Example: run once every 20 ticks
...
var EndTickCallback = Java.extend(Java.type("net.fabricmc.fabric.api.event.lifecycle.v1.ServerTickEvents.EndTick"));
var Text = Java.type("net.minecraft.text.Text");
...
#### B) Registry Lookup (Status Effect)
...
StatusEffect effect = Registries.STATUS_EFFECT.get(new Identifier("minecraft", "speed"));
```
...
var effect = Registries.STATUS_EFFECT.get(new Identifier("minecraft", "speed"));
```

#### C) Give an Item Stack
...
player.giveItemStack(new ItemStack(Items.DIAMOND, 3));
```
...
### 8) Where This Showcase Goes Further
